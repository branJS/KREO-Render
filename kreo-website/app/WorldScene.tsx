...
/**
 * KREO 3D world (refined)
 * - Soft color palette (cream brutalist)
 * - Subtle parallax motion responding to mouse
 * - Smooth sector navigation transitions
...
export default function WorldScene({ sections }: Props) {
  return (
    <div className="canvas-stage">
      ...
      >
        <Suspense fallback={<Html center><div style={{ letterSpacing: ".1em", opacity: .7 }}>loading…</div></Html>}>
...
function Scene({ sections }: Props) {
  ...
  useEffect(() => {
    const onNav = (e: any) => {
      const sec = (e?.detail?.section as string) || "home";
      const idx = Math.max(0, sections.indexOf(sec));
      const p = points[idx] ?? points[0];
      const t = 1.2;
      gsap.to(camera.position, {
        x: p.x, y: p.y + 3.5, z: p.z + 14,
        duration: t, ease: "power2.inOut"
      });
    ...
    window.addEventListener("kreo:navigate", onNav as any);
    return () => window.removeEventListener("kreo:navigate", onNav as any);
  ...
  useFrame(({ clock }) => {
    ...
    if (group.current) {
      const mx = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--mxp")) || 0.5;
      const my = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--myp")) || 0.5;
...
function BackdropPlane() {
  const uniforms = useMemo(() => ({ uC: new THREE.Color("#F2ECE3"), uTime: { value: 0 } }), []);
  useFrame((_, dt) => { (uniforms.uTime as any).value += dt; });
  return (
    <mesh rotation={[-Math.PI / 2.1, 0, 0]} position={[0, -3, -40]}>
      <planeGeometry args={[180, 180, 1, 1]} />
      <shaderMaterial uniforms={uniforms as any} vertexShader={vs} fragmentShader={fs} transparent />
    </mesh>
  );
}
...
function Plate({ color, size, offset }: { color: THREE.Color; size: [number, number, number]; offset: [number, number, number] }) {
  ...
  useFrame(({ clock }) => {
    if (ref.current) ref.current.rotation.y = Math.sin(clock.getElapsedTime() * 0.15) * 0.02;
  });
  return (
...
function Node({ color, position, label, dark }: { color: THREE.Color; position: [number, number, number]; label: string; dark?: boolean }) {
  return (
    <mesh position={position}>
      ...
      <meshStandardMaterial color={color} emissive={dark ? new THREE.Color(0x111111) : new THREE.Color(0x000000)} roughness={0.4} />
...
/* ---------------------------------------------------------------------- */
function useSectorPoints(len: number) {
  return useMemo(() => {
